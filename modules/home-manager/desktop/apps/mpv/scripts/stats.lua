...
if o.load == false then
	mp.msg.info("script is disabled")
	return
end
...
local function generate_graph(values, i, len, v_max, v_avg, scale, x_tics)
	...
	return format(
		"%s{\\rDefault}{\\pbo%f}{\\shad0}{\\alpha&H00}{\\p1}%s{\\p0}" .. "{\\bord0}{\\1c&H%s}{\\p1}%s{\\p0}%s",
		o.prefix_sep,
		y_offset,
		bg_box,
		o.plot_color,
...
local function append(s, str, attr)
	...
	s[index] = s[index] or ""
	s[index] = s[index]
		.. format("%s%s%s%s%s%s", attr.nl, attr.indent, attr.prefix, attr.prefix_sep, no_ASS(str), attr.suffix)
	return true
end
...
local function append_perfdata(header, s, dedicated_page)
	...
	for _, frame in ipairs(sorted_keys(vo_p)) do -- ensure fixed display order
		...
		if dedicated_page then
			s[#s + 1] = format("%s%s%s:", o.nl, o.indent, bold(frame:gsub("^%l", string.upper)))
...
local function get_kbinfo_lines()
	...
	for _, bind in pairs(bindings) do
		if
			...
			and (
				searched_text == nil
				or (bind.key .. bind.cmd .. (bind.comment or "")):lower():find(searched_text, 1, true)
			)
		then
	...
	local spre = term and kspaces .. "   "
		or format("{\\q2\\fn%s}%s   {\\fn%s}{\\fs%d\\u1}", o.font_mono, kspaces, o.font, 1.3 * font_size)
...
local function append_display_sync(s)
	...
	append_property(
		...
		{ prefix = "错时帧：", nl = "", indent = o.prefix_sep .. o.prefix_sep }
	)
	append_property(
		s,
		"vo-delayed-frame-count",
		{ prefix = "延迟帧：", nl = "", indent = o.prefix_sep .. o.prefix_sep }
	)
	...
	if not display_timer.oneshot and (o.plot_vsync_ratio or o.plot_vsync_jitter) and o.use_ass then
		...
		append_property(
			s,
			"vsync-jitter",
			{ prefix = "垂直同步抖动：", nl = vr and "" or o.nl, indent = vr and o.prefix_sep .. o.prefix_sep }
		)
	end
end
...
local function add_file(s, print_cache, print_tags)
	...
	if print_tags then
		...
		for _, tag in ipairs(tags) do
			...
			if
				tag ~= "Title"
				and tags_displayed < o.file_tag_max_count
				and value
				and value:len() < o.file_tag_max_length
			then
	...
	if ch_index and ch_index >= 0 then
		append_property(
			s,
			"chapter-list/" .. tostring(ch_index) .. "/title",
			{ prefix = "章节：", nl = ed_cond and "" or o.nl }
		)
	...
	append_property(
		s,
		"file-format",
		{ prefix = "封装格式/协议：", nl = fs and "" or o.nl, indent = fs and o.prefix_sep .. o.prefix_sep }
	)
...
local function crop_noop(w, h, r)
	return r["crop-x"] == 0 and r["crop-y"] == 0 and r["crop-w"] == w and r["crop-h"] == h
end

local function crop_equal(r, ro)
	return r["crop-x"] == ro["crop-x"]
		and r["crop-y"] == ro["crop-y"]
		and r["crop-w"] == ro["crop-w"]
		and r["crop-h"] == ro["crop-h"]
end
	...
	if append(s, r[w_prop], { prefix = prefix }) then
		...
		if r["aspect"] ~= nil and not video_res then
			...
			append(
				s,
				r["aspect-name"],
				{ prefix = "(", suffix = ")", nl = "", indent = " ", prefix_sep = "", no_prefix_markup = true }
			)
		end
		...
		if r["crop-w"] and (not video_res or not crop_noop(r[w_prop], r[h_prop], r)) then
			append(
				s,
				format("[x: %d, y: %d, w: %d, h: %d]", r["crop-x"], r["crop-y"], r["crop-w"], r["crop-h"]),
...
local function append_hdr(s, hdr, video_out)
	...
	if should_show(hdr["max-cll"]) or should_show(hdr["max-luma"]) then
		...
		if hdr["min-luma"] and should_show(hdr["max-luma"]) then
			...
			append(
				s,
				format("%.2g / %.0f", hdr["min-luma"], hdr["max-luma"]),
	...
	if hdr["scene-max-r"] or hdr["scene-max-g"] or hdr["scene-max-b"] or hdr["scene-avg"] then
		append(s, "", { prefix = "HDR10+:" })
		append(
			s,
			format("%.1f / %.1f / %.1f", hdr["scene-max-r"] or 0, hdr["scene-max-g"] or 0, hdr["scene-max-b"] or 0),
			{ prefix = "MaxRGB:", suffix = " cd/m²", nl = "", indent = "" }
		)
		append(
			s,
			format("%.1f", hdr["scene-avg"] or 0),
	...
	if hdr["max-pq-y"] and hdr["avg-pq-y"] then
		...
		append(
			s,
			format("%.2f cd/m² (%.2f%% PQ)", pq_eotf(hdr["max-pq-y"]), hdr["max-pq-y"] * 100),
			{ prefix = "Max:", nl = "", indent = "" }
		)
		append(
			s,
			format("%.2f cd/m² (%.2f%% PQ)", pq_eotf(hdr["avg-pq-y"]), hdr["avg-pq-y"] * 100),
...
local function add_video_out(s)
	...
	append(
		s,
		get_property_cached("display-names"),
		{ prefix_sep = "", prefix = "(", suffix = ")", no_prefix_markup = true, nl = "", indent = " " }
	)
	append(
		s,
		mp.get_property_native("current-gpu-context"),
	...
	if append_property(s, "decoder-frame-drop-count", { prefix = "丢帧暂计：", suffix = "（解码）" }) then
...
local function add_video(s)
	...
	if track then
		...
		append(
			s,
			track["codec-profile"],
			{ prefix = "[", nl = "", indent = " ", prefix_sep = "", no_prefix_markup = true, suffix = "]" }
		)
		...
		append(s, get_property_cached("hwdec-current"), {
			prefix = "硬解API：",
			nl = "",
			indent = o.prefix_sep .. o.prefix_sep,
	...
	if o.show_frame_info then
		...
		if frame_info and frame_info["interlaced"] then
			local attrs = has_prefix and { indent = " ", nl = "", prefix_sep = "" } or { prefix = "当前帧类型：" }
...
local function add_audio(s)
	...
	append_property(
		s,
		"current-ao",
		{ prefix = "当前音频输出：", nl = "", indent = o.prefix_sep .. o.prefix_sep }
	)
	...
	append(
		s,
		merge(r, ro, "format"),
		{ prefix = "编码格式：", nl = cc and "" or o.nl, indent = cc and o.prefix_sep .. o.prefix_sep }
	)
...
local function add_track(c, t, i)
	...
	local function B(b)
		return b and string.format("%.2f", b / 1024)
	end
	...
	if t["dolby-vision-profile"] or t["dolby-vision-level"] then
		...
		append(
			c,
			t["dolby-vision-level"],
			{ prefix = "Level:", nl = "", indent = t["dolby-vision-profile"] and o.prefix_sep .. o.prefix_sep or "" }
		)
	end
end
...
local function cache_stats()
	...
	append(stats, utils.format_bytes_humanized(speed) .. "/s", { prefix = "速度：", suffix = speed_graph })

	append(stats, utils.format_bytes_humanized(info["total-bytes"]), { prefix = "总计RAM大小：" })
	append(stats, utils.format_bytes_humanized(info["fw-bytes"]), { prefix = "预计RAM大小：" })
	...
	append(stats, info["bof-cached"] and "yes" or "no", { prefix = "起点已缓冲：" })
	append(stats, info["eof-cached"] and "yes" or "no", { prefix = "终点已缓冲：" })
