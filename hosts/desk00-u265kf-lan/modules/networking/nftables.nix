...
let
  ...
  inherit (config.networking.vpn.mihomo) tproxyPort mihomoMark;
  toBeTproxiedByMihomoMark = 1; # 普通流量标记
  outbounds = [
    "wlp128s20f3"
    "enp131s0"
  ];
in
{
  systemd.network.networks."50-mihomo" = {
    name = "lo";
    routes = [
      {
        Scope = "host";
    ...
    routingPolicyRules = [
      {
        Family = "both";
        FirewallMark = builtins.toString toBeTproxiedByMihomoMark;
        Priority = 10;
        Table = 100;
      }
    ];
    ...
    nftables.tables."sys-fw" = {
      ...
      content = ''
          ...
          chain output {
            ...
            oifname != {${builtins.concatStringsSep "," outbounds}} accept                                   # 绕过本机内部通信的流量（接口lo）
            meta mark ${builtins.toString mihomoMark} accept                                   # 绕过本机clash发出的流量
            fib daddr type { local, broadcast, anycast, multicast } accept # 绕过本地、单播、组播、多播地址
